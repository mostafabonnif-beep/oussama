...
class PipController(private val activity: Activity) {
    ...
    fun update(isPlaying: Boolean, canZap: Boolean) {
        ...
        if (canZap) {
            builder.setActions(
                listOf(
                    remoteAction(ACTION_PREV, "القناة السابقة", android.R.drawable.ic_media_previous),
                    remoteAction(ACTION_NEXT, "القناة التالية", android.R.drawable.ic_media_next)
                )
            )
        }
    ...
    companion object {
        private const val ACTION_PIP_CONTROL = "com.dzhoof.iptv.PIP_CONTROL"
